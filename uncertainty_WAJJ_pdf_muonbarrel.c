...
Int_t histo_number=103;//the histo_number equals the amount of pdf except the central value
Double_t central_value[12],nlo_value[103][12];//[the amount of pdf][the amout of bin]
ofstream f1("./result_muonbarrel/WAJJ-pdf-muonbarrel-uncertainty.txt");
...
void open(Int_t i){
    ifstream file(Form("./scale-txt-WAJJ-pdf-muonbarrel/content-hist_no%d",i));
...
int uncertainty_WAJJ_pdf_muonbarrel(){
    gStyle->SetOptStat(0);
    f1<<"Bins"<<"\t"<<"pdf uncertainty"<<endl;
...
//	f1<<"bin "<<j+1<<"\t"<<uncertainty[j]<<endl;
    ...
    for(Int_t i=0;i<12;i++){
        ...
        h1->SetTitle("WAJJ pdf uncertainty_muonbarrel;;uncertainty");
        ...
        TArrow* line1 = new TArrow(h1->GetBinLowEdge(5),0.032,h1->GetBinLowEdge(5),h1->GetMaximum(),0.07,"") ;
        TArrow* line2 = new TArrow(h1->GetBinLowEdge(9),0.032,h1->GetBinLowEdge(9),h1->GetMaximum(),0.07,"") ;
