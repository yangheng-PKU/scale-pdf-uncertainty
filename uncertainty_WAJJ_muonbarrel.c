...
ofstream f1("./result_muonbarrel/WAJJ_muonbarrel_scale_uncertainty.txt");
...
void open(Int_t i){
    ifstream file(Form("./scale-txt-WAJJ-muonbarrel/content-hist_no%d",i));
...
int uncertainty_WAJJ_muonbarrel(){
    gStyle->SetOptStat(0);
...
//	f1<<"bin "<<j+1<<"\t"<<uncertainty[j]<<endl;
    ...
    for(Int_t i=0;i<12;i++){
        ...
        h1->SetTitle("WAJJ scale uncertainty_muonbarrel;;uncertainty");
        ...
        TArrow* line1 = new TArrow(h1->GetBinLowEdge(5),0.01,h1->GetBinLowEdge(5),h1->GetMaximum(),0.08,"") ;
        TArrow* line2 = new TArrow(h1->GetBinLowEdge(9),0.01,h1->GetBinLowEdge(9),h1->GetMaximum(),0.08,"") ;
