...
int unroll_WAJJ_muonbarrel(){
    ...
    TFile* f_WAJJ=TFile::Open("./th2-histo-WAJJ-muonbarrel.root");
    TH2D* th2_WAJJ[7];
    TH1D* t_WAJJ[7];
    ...
    for(Int_t i=0;i<7;i++){
        ...
        th2_WAJJ[i]=(TH2D*)f_WAJJ->Get(Form("th2_%d",i));
        t_WAJJ[i]= unroll(th2_WAJJ[i], mjj_bins, detajj_bins, 4,3,Form("hist_%d",i+1));//Form("%d central scale pdf variable",i+1));
        ofstream file2(Form("./scale-txt-WAJJ-muonbarrel/content-hist_no%d",i+1));
        t_WAJJ[i]->SetLineWidth(3);
        t_WAJJ[i]->SetLineColor(i+1);
        for(Int_t j=1;j<=12;j++){ t_WAJJ[i]->GetXaxis()->SetBinLabel(j,name[j-1]);}
        t_WAJJ[i]->Scale(lumi);
        ...
        for(Int_t k=0;k<12;k++){
            file2<<t_WAJJ[i]->GetBinContent(k+1)<<endl;}
    }
    //ll->Draw();
    ...
    TLegend *l2 = new TLegend(0.6,0.6,0.8,0.9);
    t_WAJJ[0]->SetTitle("Mjj vs Mla");
    t_WAJJ[0]->SetLineWidth(3);
    t_WAJJ[0]->Draw("HIST");
   // file1<<"histo 1 1"<<endl;
    for(Int_t i=1;i<7;i++){
        ...
        t_WAJJ[i]->SetLineColor(i+1);
        t_WAJJ[i]->SetLineWidth(1);
        t_WAJJ[i]->SetLineStyle(2);
        //t_WA[i]->Scale(lumi*WA_scale);
       // file1<<"histo "<<i+1<<"\t"<<t_WAJJ[0]->Integral()/t_WAJJ[i]->Integral()<<endl;
        cout<<"histo "<<i+1<<" "<<t_WAJJ[0]->Integral()/t_WAJJ[i]->Integral()<<endl;
        t_WAJJ[i]->Scale(t_WAJJ[0]->Integral()/t_WAJJ[i]->Integral());
        for(Int_t j=1;j<=12;j++){ t_WAJJ[i]->GetXaxis()->SetBinLabel(j,name[j-1]);}
        //t_ZA[i]->Draw("HIST,SAME");
        t_WAJJ[i]->Draw("HIST,SAME");
        //t_WA[i]->DrawNormalized("HIST");
        l2->AddEntry(t_WAJJ[i],Form("%d central scale pdf variations",i+1));
        // delete t_ZA[i];
        // delete cc[i];
    }
    l2->Draw();
   // c1->Print("hist-2d-WAJJ.eps");
   ...
   // TLegend *l1 = new TLegend(0.55,0.4,0.8,0.9);
    TLegend *l1 = new TLegend(0.2,0.8,0.6,1.2);
    for(Int_t i=0;i<7;i++){
        hs->Add(t_WAJJ[i]);
        l1->AddEntry(t_WAJJ[i],Form("%d central scale pdf variations",i+1));
    }
    /*
