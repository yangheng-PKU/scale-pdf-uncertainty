...
ofstream f1("./result_muonbarrel/WA_muonbarrel_pdf_uncertainty.txt");
...
void open(Int_t i){
    ifstream file(Form("./scale-txt-WA-pdf-muonbarrel/content-hist_no%d",i));
...
int uncertainty_WA_pdf_muonbarrel(){
    gStyle->SetOptStat(0);
...
//	f1<<"bin "<<j+1<<"\t"<<uncertainty_other[j]<<endl;
    ...
    for(Int_t i=0;i<12;i++){
        ...
        h1->SetTitle("WA pdf uncertainty_muonbarrel;;uncertainty");
        ...
        TArrow* line1 = new TArrow(h1->GetBinLowEdge(5),0.010,h1->GetBinLowEdge(5),h1->GetMaximum(),0.07,"") ;
        TArrow* line2 = new TArrow(h1->GetBinLowEdge(9),0.010,h1->GetBinLowEdge(9),h1->GetMaximum(),0.07,"") ;
