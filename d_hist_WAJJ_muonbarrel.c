...
void run(std::string filename){
    ...
    */
    ...
    for(Int_t j=0; j<t->GetEntries();j++)
    {
        ...
        t->GetEntry(j);
        //gai
        for(Int_t i=104;i<111;i++){
        if(i<109){
       // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];}
        if(i==109){
       // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i+1];}
        if(i==110){
       //                        // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i+2];}
   // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        /* else*/  
//	actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
...
int d_hist_WAJJ_muonbarrel(){
    run("outWAJJ");
    //TFile* f4=new TFile("2d_WA.root","RECREATE");
    
    TFile* f4=new TFile("th2-histo-WAJJ-muonbarrel-tmp.root","RECREATE");
    for(Int_t i=0;i<7;i++){
        th2[i]->Write();}
    f4->Close();
    //f5->Close();
