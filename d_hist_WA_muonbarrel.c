...
void run(std::string filename){
    ...
    */
	...
	if(i==109){
       // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        actualWeight[p]=2*scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i+1];}
        if(i==110){
       // if(p==0)  actualWeight[p]=scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i];
        actualWeight[p]=2*scalef*L1prefiring*lumiWeight*pileupWeight*pweight[i+2];}
...
int d_hist_WA_muonbarrel(){
    run("outWA");
    //TFile* f4=new TFile("2d_WA.root","RECREATE");
    
   // TFile* f4=new TFile("th2-histo-WA-muonbarrel.root","RECREATE");
    TFile* f4=new TFile("th2-histo-WA-muonbarrel-tmp.root","RECREATE");
    for(Int_t i=0;i<7;i++){
        th2[i]->Write();}
    f4->Close();
    //f5->Close();
